...
package net.mcreator.genshincore.world.features;
...
public class PumpkinPatchSpookyFeature extends Feature<NoneFeatureConfiguration> {
	...
	public PumpkinPatchSpookyFeature() {
		super(NoneFeatureConfiguration.CODEC);
		base_blocks = List.of(GenshinCoreModBlocks.MONDSTADT_MOUNTAIN_GRASS_BLOCK.get());
	}
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("genshin_core", "pumpkin_patch_spooky_1"));
		if (template == null)
			return false;
